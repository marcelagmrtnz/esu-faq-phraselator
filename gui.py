...
from phraselate import import_dict, import_vectors, import_questions
from phraselate import translate, select_question
...
def run_tts(filename):
    ...
    command = ['deepspeech', '--model', '../resources/deepspeech-0.7.3-models.pbmm', '--scorer', '../resources/deepspeech-0.7.3-models.scorer', '--audio', sox_filename]
    process = subprocess.run(command, capture_output=True)
    output = process.stdout.decode()

    return output.strip()

def audio_to_question(filename, fps, use_translations):
    ...
    if use_translations:
        translation = translate(query.split(), import_dict(sys.argv[1]), import_vectors(sys.argv[2]))
        questions = select_question(translation, import_questions(sys.argv[3]))
    else:
        questions = select_question(query.split(), import_questions(sys.argv[3]), translate=False)
    
    return questions

def main():
    questions = []
    with open(sys.argv[3], 'r') as question_file:
    ...
    [simple.Text("If you choose the 'Translated Search' option below, the system will search for the closest question in Yup'ik by first translating your question. This may hinder performance.\nThe 'English Search' option performs better and faster, but searches the questions with your direct input. Either way, you will get questions and answers in both languages.", font='25')],
    [simple.Radio('Translated Search', 'translate', key='translate', font='15'), simple.Radio('English Search', 'translate', default=True, key='english', font='15')],
    ...
    while True:
        ...
        if gui_event == simple.WIN_CLOSED:
            ...
            questions = audio_to_question('query.wav', fps, values['translate'])
